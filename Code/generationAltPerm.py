...
# using the method described in Marchal's paper
...
def update(x):
    return 1 - (2 / math.pi) * math.asin(random.random() * math.sin(math.pi * x/2))

# helper function for calculating acceptance
def calculateAcceptanceProb(x_n, x_0):
    alpha = math.sin(math.pi * x_n/ 2) / math.sin(math.pi * x_0/ 2)
